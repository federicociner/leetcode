...
class Logger:
    ...
    def shouldPrintMessage(self, timestamp: int, message: str) -> bool:
        ...
        """
        if timestamp < self.logs.get(message, 0):
            return False
...
if __name__ == "__main__":
    ...
    assert logger.shouldPrintMessage(0, "B") is True
    assert logger.shouldPrintMessage(0, "C") is True
    assert logger.shouldPrintMessage(0, "A") is False
    assert logger.shouldPrintMessage(0, "B") is False
    assert logger.shouldPrintMessage(0, "C") is False
    assert logger.shouldPrintMessage(11, "A") is True
    assert logger.shouldPrintMessage(11, "B") is True
    assert logger.shouldPrintMessage(11, "C") is True
    assert logger.shouldPrintMessage(11, "A") is False
